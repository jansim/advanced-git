...
- You can have repositories inside repositories, these are called *submodules*
- Add one with `git submodule add <repo-url> <path>` (similar to a clone)
- Your main project just stores the submodule's path and its current commit hash
  - The repository tracks its own (separate) history
- In practice their usefulness can be limited
