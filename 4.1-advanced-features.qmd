...
- Modify the last commit: Change commit message or content
- `git commit --amend`
- **DANGER: Never do this if you already pushed the commit**

## Staging Parts Instead of full Files

Instead of staging complete files, you can also stage just parts of them.

- CLI
  - `git add --patch <file>`
  - `git add -p <file>`
- VSCode: "Stage selected range"
  - Demo
...
- CLI: `git tag -a v1.2.3`
- VSCode: Demo

Note: Tags need to be pushed *explicitly* via `git push --tags`

<!-- Optional: Practical - Creating a tag & release -->
<!-- Optional: Practical: learngitbranching > "A Mixed Bag" > #4: Tags -->

# Patches 🩹

## Patches 🩹

Before platforms like GitLab/Hub existed, people collaborated by sending around **patches** via email. A patch is essentially a commit exported as a file.

- Create: `git format-patch <commit/branch>`
  - Will create `.patch` files for all commits till target
- Apply: `git apply <patch-file>`
...
<!--
  If extra time: https://learngitbranching.js.org/
  Section 2 "Ramping up"
  Section 3: Moving Work around #1 (cherry pick)
-->
